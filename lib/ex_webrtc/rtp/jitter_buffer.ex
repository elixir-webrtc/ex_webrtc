defmodule ExWebRTC.RTP.JitterBuffer do
  ...
  * `buffer` - `t:#{inspect(__MODULE__)}.t/0`.
  ...
  def handle_timeout(%__MODULE__{} = buffer) do
    %{buffer | state: :timer_not_set} |> send_packets()
  end

  @spec try_insert_packet(t(), Packet.t()) :: {:ok | :error, t()}
  defp try_insert_packet(%__MODULE__{store: store} = buffer, packet) do
    case PacketStore.insert(store, packet) do
      {:ok, store} -> {:ok, %{buffer | store: store}}
      {:error, :late_packet} -> {:error, buffer}
    end
  end

  @spec send_packets(t()) :: result()
  defp send_packets(%__MODULE__{store: store} = buffer) do
    ...
    {buffer, timer} = maybe_set_timer(%{buffer | store: store})
  ...
  defp maybe_set_timer(%__MODULE__{state: :initial_wait, store: store} = buffer) do
  ...
  defp maybe_set_timer(%__MODULE__{state: :timer_not_set, store: store} = buffer) do
    case PacketStore.first_packet_timestamp(store) do
      nil ->
        {buffer, nil}
      ...
      timestamp_ms ->
        ...
        {%{buffer | state: :timer_set}, send_after_time}
    end
  end

  defp maybe_set_timer(%__MODULE__{state: :timer_set} = buffer), do: {buffer, nil}
end
