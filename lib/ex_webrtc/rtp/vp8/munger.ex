defmodule ExWebRTC.RTP.VP8.Munger do
  ...
  @spec init(t(), ExRTP.Packet.t()) :: t()
  def init(vp8_munger, packet) do
    {:ok, vp8_payload} = VP8.Payload.parse(packet.payload)
  ...
  def update(vp8_munger, packet) do
    {:ok, vp8_payload} = VP8.Payload.parse(packet.payload)
    ...
    pic_id_offset = (vp8_munger.last_pic_id && pic_id - vp8_munger.last_pic_id - 1) || 0

    tl0picidx_offset =
      (vp8_munger.last_tl0picidx && tl0picidx - vp8_munger.last_tl0picidx - 1) || 0

    keyidx_offset = (vp8_munger.last_keyidx && keyidx - vp8_munger.last_keyidx - 1) || 0
  ...
  @spec munge(t(), ExRTP.Packet.t()) :: {t(), ExRTP.Packet.t()}
  def munge(vp8_munger, %ExRTP.Packet{payload: <<>>} = packet), do: {vp8_munger, packet}

  def munge(vp8_munger, packet) do
    {:ok, vp8_payload} = VP8.Payload.parse(packet.payload)
    ...
    packet = %ExRTP.Packet{packet | payload: vp8_payload}
